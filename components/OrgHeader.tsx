...
import { ChevronsUpDown, Settings, Users, Building } from "lucide-react";

const OrgHeader = ({org}:{org:any}) => {
  return (
    <header className="flex justify-between items-center bg-neutral-900 text-neutral-100 px-6 py-3 border-b border-gray-800/70 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="font-medium text-sm text-gray-100">{org.name}</p>
           <button className="p-2  text-sm font-light border border-gray-800 rounded-xl  transition-colors group">
            {org.plan}
          </button>
          <ChevronsUpDown className="w-4 h-4 text-gray-400 cursor-pointer hover:text-gray-300" />
        </div>
      </div>
